...
async function main() {
  ...
  for (const f of smithyFiles) {
    if (f.isFile() && f.name.endsWith('.json')) {
      ...
      services.push(...await readServicesFromModelFile(smithy));
    }
  }
...
async function readServicesFromModelFile(model: SmithyModel): Promise<AWSService[]> {
  const services = Object.entries(model.shapes ?? {})
    .flatMap(([id, shape]) => isService(shape) ? [parseService(id, shape, model)] : []);

  return services;
}

function parseService(id: string, smithyService: Service, model: SmithyModel): AWSService {
  const name = smithyService.traits?.['smithy.api#title'] ?? id;
  const shortName = smithyService.traits?.['aws.api#service']?.sdkId ?? id;
